...
class LibraryPickerScreen(ModalScreen[Path | None]):
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="dialog"):
            yield Static("Musik-Verzeichnis waehlen", id="dialog-title")
            yield Label(
                "Wo liegt deine Musiksammlung?",
                id="dialog-hint",
            )
            ...
            yield Static("oder eigenen Pfad eingeben:", id="separator")
            yield Input(
                placeholder="z.B. D:\\Dropbox\\Musik",
                id="custom-path",
            )
            yield Button(
                "Uebernehmen",
    ...
    def _accept_custom(self) -> None:
        ...
        if not raw:
            error_label.update("Bitte einen Pfad eingeben.")
            error_label.add_class("visible")
            return

        path = Path(raw).expanduser().resolve()
        if not path.is_dir():
            error_label.update(f"Verzeichnis nicht gefunden: {path}")
            error_label.add_class("visible")
            return
