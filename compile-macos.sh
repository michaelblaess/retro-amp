#!/usr/bin/env bash
# compile-macos.sh - compiles retro-amp into a standalone macOS binary with Nuitka.
#
...
# and dist/retro-amp-vX.Y.Z-macos-<arch>.tar.gz ready to hand out.
#
...
#     xattr -dr com.apple.quarantine retro-amp
...
echo "Compiling retro-amp v$version ($arch) with Nuitka..."
...
tarball="$out_dir/retro-amp-v$version-macos-$arch.tar.gz"
rm -f "$tarball"
